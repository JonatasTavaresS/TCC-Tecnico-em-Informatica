...
contract Donations {
    ...
    */
    ...
    // Cadastrar Estudante
    function registerStudent(address adStudent, uint goal) public returns(bool registered){
        registered = false;
        
        // Verificar se o Estudante está cadastrado na lista de Doadores
        bool verifyDonor = false;
        for (uint i = 0; i < students.length; i++) {
            if (donors[i].donor == adStudent) {
                verifyDonor = true;
            }
        }
        
        // Verificar se o Estudantes já está cadastrado na lista de Estudantes
        bool verifyStudent = false;
        for (i = 0; i < donors.length; i++) {
    ...
    // Verificar se o Estudante foi cadastrado
    function getStudent(address adStudent) public view returns(bool registered) {
        registered = false;
        for (uint i = 0; i < students.length; i++) {
            if (students[i].student == adStudent) {
                registered = true;
    ...
    // Verificar se o Doador foi cadastrado
    function getDonor() public view returns(bool registered) {
        registered = false;
    ...
    // Verificar o valor da última doação
    function getLastDonation(address adStudent) public view returns(uint lastDonation) {
        for (uint i = 0; i < students.length; i++) {
            if (students[i].student == adStudent) {
                lastDonation = students[i].donations[students[i].donations.length - 1].quantity;
            }
        }
    }
    ...
    function checkCollected(address adStudent) public view returns(uint collected) {
        for (uint i = 0; i < students.length; i++) {
            if (students[i].student == adStudent) {
                collected = students[i].collected;
            }
        }
    }
}